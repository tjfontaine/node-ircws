...
Throttle.prototype._transform = function thrtlTransform(client, enc, cb) {
  ...
  var time = CONNECTIONS[client.remoteParsedAddressString];
  var cur = Date.now();
  var timeout = this.thrt_timeout;

  if (time && (cur - time) < timeout) {
    this.log.error('throttling client', {
      remoteAddress: client.remoteParsedAddressString,
      ts: cur,
      prev: time,
      timeout: timeout,
    });
  ...
  this.log.trace('client time registered', { remoteAddress: client.remoteParsedAddressString, ts: cur });
  CONNECTIONS[client.remoteParsedAddressString] = cur;
  this.push(client);
  return cb();
};
