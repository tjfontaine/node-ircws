...
function SIOServer(options) {
  ...
  this.sio_server.on('secureConnection', function sioClient(socket) {
    var addr = socket.address();
    addr.remoteAddress = socket.remoteAddress;
    addr.remotePort = socket.remotePort;
    self.log.trace({ addr: addr }, 'secure connect');
    socket.on('close', function sioSocketClose() {
      self.log.trace({ addr: addr }, 'underlying socket closed');
    });
  });
...
function SIOClient(client) {
  ...
  this.sioc_client.on('disconnect', function SIOdisconnect() {
    self.log.trace('socketio disconnect');
    self.emit('close');
  });

  // socketio doesn't extend Socket it encapsulates it
  if (client.client && client.client.conn) {
    var socket = client.conn.request.socket;
...
SIOClient.prototype.destroy = function SIOdestroy() {
  ...
  var logger = this.log ? this.log.trace.bind(this.log) : console.error;

  if (this.sioc_client.destroy) {
    logger('using destroy for client');
    this.sioc_client.destroy();
  } else if (this.sioc_client.close) {
    logger('using close for client');
    this.sioc_client.close();
